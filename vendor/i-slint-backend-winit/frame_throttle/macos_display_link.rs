...
pub(super) fn try_create(
    ...
    // HorizonCast local patch: the `CADisplayLink` *class* exists on macOS < 14, but the
    // `-[NSView displayLinkWithTarget:selector:]` method it uses below is macOS 14+. The
    // class check above is therefore not enough — verify the view actually responds to the
    // selector, and fall back to the timer-based throttle (return None) otherwise, instead
    // of aborting with "unrecognized selector" on macOS 12/13.
    let responds: bool =
        unsafe { msg_send![ns_view, respondsToSelector: sel!(displayLinkWithTarget:selector:)] };
    if !responds {
        return None;
    }
